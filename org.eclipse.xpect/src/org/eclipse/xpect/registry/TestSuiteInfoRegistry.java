...
public class TestSuiteInfoRegistry implements ITestSuiteInfo.Registry {
	...
	public TestSuiteInfoRegistry() {
		...
		for (IExtensionInfo info : IExtensionInfo.Registry.INSTANCE.getExtensions(ORG_ECLIPSE_XPECT_TEST_SUITE)) {
			...
			if (clazz == null) {
				LOG.warn("Attribute 'class' missing for extension '" + ORG_ECLIPSE_XPECT_TEST_SUITE + "' in " + info.getLocation());
				continue;
			}
			String fileExtension = info.getAttributeValue("fileExtension");
			if (fileExtension == null) {
				LOG.warn("Attribute 'fileExtension' missing for extension '" + ORG_ECLIPSE_XPECT_TEST_SUITE + "' in " + info.getLocation());
				continue;
			}
